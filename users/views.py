...
def login_request(request):
    if request.method == 'POST':
        ...
        print(api_key)
        req = requests.get(f'https://api.torn.com/user/?selections=basic&key={api_key}')
        print(req)
        ...
        if data.get('error') is not None:
            ...
            (print(player_id in Profile.objects.all().values('torn_id')),'TEEEEEST \n \n \n \n \n')
            if player_id in [a['torn_id'] for a in Profile.objects.values('torn_id')]: # login
                print('logging IN ! \n \n \n')
