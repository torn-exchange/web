...
class Profile(VoteModel, models.Model, HitCountMixin):
    ...
    def save(self, *args, **kwargs):
        try:
            self.work_stats_total = self.work_stats_end + self.work_stats_int + self.work_stats_man
        except TypeError:
...
class Settings(models.Model):
    owner = models.OneToOneField(Profile, on_delete=models.CASCADE, related_name='settings')
    trade_list_description = models.CharField(max_length=500, null=True,blank=True)
    link_to_forum_post = models.CharField(max_length=250, null=True,blank=True)
    receipt_paste_text = models.TextField(max_length=250, null=True,blank=True)
    ...
    selling_company_description = models.CharField(max_length=140, null=True, blank=True)   
    company_looking_to_hire = models.BooleanField(default=False)
    company_looking_to_hire_message = models.CharField(max_length=140, null= True,blank=True)
    __job_seeking = None
    ...
    def save(self, *args, **kwargs):
        if self.pk:
            ...
            self.__job_seeking = self.job_seeking
        players_company = apps.get_model('main','Company').objects.filter(owner=self.owner).first()
