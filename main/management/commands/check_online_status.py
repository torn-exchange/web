...
class Command(BaseCommand):
    def _update(self):
        ...
        active_traders = set([a.owner.user for a in Listing.objects.filter().distinct('owner')])
        ...
        Profile.objects.filter(settings__selling_company = True)
        ])
        ...
        for profile in Profile.objects.filter(user__in= users_to_be_checked):
            if profile.api_key != '':
                    req = requests.get(f'https://api.torn.com/user/?selections=profile&key={profile.api_key}')
...
def update_companies():
    profiles_to_update = [obj.owner for obj in Settings.objects.filter(Q(company_looking_to_hire=True) | Q(selling_company=True))]
    for profile in profiles_to_update:
        update_company_sale_data(profile)
        profile.save()
    
def update_workstats_data(profile):
    work_data = json.loads(requests.get(f'https://api.torn.com/user/?selections=workstats&key={profile.api_key}').content)
...
def update_company_sale_data(profile):
    ...
    try:
        name, company_id, company_type, rating, days_old, weekly_income, weekly_customers, employees_hired,employees_capacity,popularity,efficiency,average_employee_efficiency, average_employee_tenure = get_company_info(profile.api_key)
        print('Sucess')
...
def get_company_info(api_key):
        ...
        req = requests.get(f'https://api.torn.com/company/?selections=detailed,profile,employees&key={api_key}')
        data = json.loads(req.content)
        if data.get('error') is not None:
            ...
            average_employee_tenure = int(np.round(np.mean([data['company_employees'][a]['days_in_company'] for a in data['company_employees']] )))
            average_employee_efficiency = int(np.round(np.mean([data['company_employees'][a]['effectiveness']['working_stats'] for a in data['company_employees']])))
