...
class Command(BaseCommand):
    ...
    req = requests.get(f'https://api.torn.com/torn/?selections=items&key={system_api_key}')
    ...
    def _populate(self,df=df):
        ...
        for index, row in df.iterrows():
            if row['circulation'] < project_settings.MINIMUM_CIRCULATION_REQUIRED_FOR_ITEM:
                continue
            item_id = row['image'].replace('https://www.torn.com/images/items/','').replace('/large.png','')
            #print(row)
            TE_price = get_lowest_market_price(item_id,get_random_key(),row['market_value'])
            ...
            if (item_in_our_db != None):
                if item_in_our_db.TE_value != TE_price:
                    ...
                    print(f'Updated {row["name"]} -{item_id} market price to {row["market_value"]} and TE_price to {TE_price}')
            else:
                Item.objects.update_or_create(
                    name = row['name'],
                    defaults=dict(
...
def get_lowest_market_price(item_id,api_key,avg_market_price=np.nan):
    time.sleep(0.08)
    req = requests.get(f'https://api.torn.com/market/{item_id}?selections=itemmarket,bazaar&key={api_key}')
    ...
    if api_key != '':
        if data.get('error'):
            ...
            if bazaar_data is not None: 
                try:
                    first_three = list(map( lambda x: x.get('cost'), bazaar_data[:3]))
            ...
            if itemmarket_data is not None: 
                try:
                    first_three = list(map( lambda x: x.get('cost'), itemmarket_data[:3]))
            ...
            try:
                TE_price = int(round(np.nanmin(pricing_data[np.nonzero(pricing_data)])))
...
def get_points_market_value():
    system_api_key = os.getenv('SYSTEM_API_KEY')
    req = requests.get(f'https://api.torn.com/market/?selections=pointsmarket&key={system_api_key}')
    data = json.loads(req.content)
    points_cost = int(round(np.nanmean([data['pointsmarket'][a]['cost'] for a in data['pointsmarket']][0:5])))
    #print(points_cost)
    return points_cost
