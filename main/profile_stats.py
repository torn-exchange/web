...
def return_profile_stats(profile):
    ...
    if has_data:
        ...
        last_week_trades = ItemTrade.objects.filter(owner=profile, last_updated__gte=datetime.now()-timedelta(days=7))
        last_week_receipts = TradeReceipt.objects.filter(owner=profile, created_at__gte=datetime.now()-timedelta(days=7))
        ...
        top_sellers = df_data_receipts.groupby('seller').count()['tmp'].sort_values(ascending=False)[:10].to_dict()
        
        def get_total_profit(name):
            return(df_data_trades[df_data_trades['seller']==name]['profit'].sum())

        top_profits =  {name : get_total_profit(name) for name in top_sellers.keys()}


        
        data_graph_2(df_data_trades)
        try:
            last_week_days = df_data_trades_last_week.groupby('created_at').count().index.values.tolist()
        except:
            last_week_days = []
        count_last_week = df_data_receipts_last_week.groupby('created_at').count()['tmp'].values.tolist()
   
        last_week_profits = df_data_trades_last_week.groupby('created_at').sum()['profit'].values.tolist()
