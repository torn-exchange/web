...
class Listing(models.Model):
    ...
    def effective_price(self):
        ...
        if (self.discount is not None) and (self.price is not None):
            discount_fraction = (100.0-(self.discount))/100.0
            discount_price = discount_fraction*round(self.item.TE_value or 0)
            effective_price = round(np.nan_to_num(np.nanmin([discount_price,self.price])))
...
class ItemTrade(models.Model):
    ...
    @property
    def profit(self):
        profit = (self.TE_value_at_save*self.quantity)-(self.price * self.quantity)
        #print(profit)
        return profit  
...
class TradeReceipt(models.Model):
    ...
    created_at = models.DateTimeField(auto_now_add=True)
    receipt_url_string = models.CharField(max_length=10, null=False, default=generate_url_string, unique=True)
    def __str__(self):
