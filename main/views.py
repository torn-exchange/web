...
def homepage(request):
    ...
    print(top_20)
    created_today = Profile.objects.filter(created_at__gte=datetime.now()-timedelta(days=365)).count()
    changes_this_week = ChangeLog.objects.filter(created_at__gte=datetime.now()-timedelta(days=365)).order_by('-created_at')
    changes_this_month = ChangeLog.objects.filter(created_at__gte=datetime.now()-timedelta(days=30)).order_by('-created_at')
    try:
...
def employee_listings(request):
    try:
        ...
        profile = None
        user_settings = None
    queryset = Profile.objects.filter(settings__job_seeking = True).all().order_by('last_active')
...
def company_listings(request):
    try:
        ...
        profile = None
        user_settings = None
    queryset = Company.objects.filter(owner__settings__selling_company=True).order_by('-rating')
...
def company_hiring_listings(request):
    try:
        ...
        profile = None
        user_settings = None
    queryset = Company.objects.filter(owner__settings__company_looking_to_hire=True).all().order_by('-rating')
    myFilter = CompanyListingFilter(request.GET,queryset=queryset)
    qs = myFilter.qs
    paginator = Paginator(qs,4)
    page = request.GET.get('page')
...
def revives_listings(request):
    ...
    revivers = Profile.objects.filter(settings__selling_revives = True).all().order_by(F('last_active').desc(nulls_last=True))
...
def losses_listings(request):
    ...
    loss_sellers = Profile.objects.filter(settings__selling_losses = True).all().order_by(F('last_active').desc(nulls_last=True))
...
def edit_price_list(request):
    profile = Profile.objects.filter(user = request.user).get()
    all_relevant_items = Item.objects.filter(circulation__gt=project_settings.MINIMUM_CIRCULATION_REQUIRED_FOR_ITEM,TE_value__gt=10).order_by('-TE_value')
    relevant_item_dict = all_relevant_items.values("name","TE_value","image_url")
    ...
    for category in categories:
        data_dict.update({category:Item.objects.filter(item_type = category,circulation__gt=project_settings.MINIMUM_CIRCULATION_REQUIRED_FOR_ITEM,TE_value__gt=10).order_by('-TE_value')})
...
def price_list(request, identifier=None):
    ...
    if Profile.objects.filter(torn_id=identifier).exists(): #if the torn_id for the page corresponds to an existing profile
        try:
            profile = Profile.objects.filter(user = request.user).get() # fetches the profile of the visiting user
        except:
            profile = None
        pricelist_profile = Profile.objects.filter(torn_id = identifier).get() # fetches the profile of the pricelist owner


    elif Profile.objects.filter(name=identifier).exists():  # fetches the profile of the visiting user using profile name 
        try:
            profile = Profile.objects.filter(user = request.user).get()
        except:
            profile = None
...
#####
    ...
    try:
        ...
        last_updated = None
    all_relevant_items = Item.objects.filter(listing__in=Listing.objects.filter(owner = pricelist_profile)).all().order_by('-TE_value')
    distinct_categories = [a['item_type'] for a in all_relevant_items.values('item_type').distinct()]
...
def calculator(request):
    ...
    all_relevant_items = Item.objects.filter(listing__in=Listing.objects.filter(owner = profile)).all()
...
def parse_trade_paste(request):
    if request.method == "POST":
        ...
        if trade_paste is not None:
            ...
            item_urls =[Item.objects.filter(name=a).get().image_url for a in item_list ]
            market_prices = [Item.objects.filter(name=a).get().TE_value for a in item_list ]
        ...
        try:
            ...
            for i in items:
                ...
                try:
                    ...
                    listings.append(Listing.objects.get(owner=profile, item__name=i))
            ...
            for i in range(len(listings)):
                try:
                    profit_per_item.append(listings[i].profit_per_item*quantities[i])
                except Exception as e:
                    print(e)
                    profit_per_item.append(0)
            image_url = [a.image_url if a is not None else '' for a in items_objects]
            market_values = [a.TE_value if a is not None else 0 for a in items_objects]
...
def create_receipt(request):
    if request.method=="POST":
        ...
        try:
            ...
            owner_profile = Profile.objects.filter(user__username = owner_user_id).get()
        ...
        for i in range(len(item_names)):
            ...
            item_trade = ItemTrade(owner=owner_profile,item=item,price=price,quantity=quantity, seller=seller_name)
...
def new_create_receipt(request):
    if request.method=="POST":
        ...
        trade_receipt.save()
        #print('receipt has been saved')
        listings_count = TradeReceipt.objects.filter(owner=owner_profile,seller=seller_name).count()
        trade_paste_text = owner_profile.settings.receipt_paste_text
        trade_paste_text = trade_paste_text.replace('[[seller_name]]',seller_name)
        trade_paste_text = trade_paste_text.replace('[[total]]',str(trade_receipt.total))
        trade_paste_text = trade_paste_text.replace('[[receipt_link]]',f'www.tornexchange.com/receipt/{trade_receipt.receipt_url_string}')
        trade_paste_text = trade_paste_text.replace('[[trade_number]]',str(listings_count))
        trade_paste_text = trade_paste_text.replace('[[prices_link]]',f'www.tornexchange.com/prices/{owner_profile.name}')
        trade_paste_text = trade_paste_text.replace('[[forum_link]]',f'www.torn.com/{owner_profile.settings.link_to_forum_post}')
